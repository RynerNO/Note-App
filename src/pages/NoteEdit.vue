<template lang="pug">
  div
    ...
    aside.noteSidebar.noteSidebarToggle(ref="sidebar")
      div.noteSidebarNav
      div.noteSidebarNavButton(@click="currentFilter = 'activeTasksFilter'; currentFilterText = 'Активно'" :class="{'noteSidebarNavButtonActive': currentFilter === 'activeTasksFilter'}") 
        img(src="../assets/NoteSidebarActive.svg")
        |Активно 
      div.noteSidebarNavButton(@click="currentFilter = 'plannedTaskFilter'; currentFilterText = 'Запланировано'" :class="{'noteSidebarNavButtonActive': currentFilter === 'plannedTaskFilter'}")  
        img(src="../assets/NoteSidebarPlanned.svg")
        |Запланировано
      div.noteSidebarNavButton(@click="currentFilter = 'completedTaskFilter'; currentFilterText = 'Выполнено'" :class="{'noteSidebarNavButtonActive':currentFilter === 'completedTaskFilter'}") 
    ...
    div.noteContent.noteContentToggle(ref="content")
      div.titleContainer
        ...
        hr.titleUnderline
      p.currentFilter {{ currentFilterText }}
      div.tasksContainer
        div(v-for="task of filter" :key="task.id")
          
            task-view( :id="task.id" :text="task.text" :priority="task.priority" :dateCompleted="task.dateCompleted"  :completed="task.completed" :date="task.date" :subTasks="task.subTasks" @taskChanged="changeTask($event)" @taskDelete="deleteTask(task.id)" @subTaskDelete="deleteSubTask(task.id, $event)")
    ...
    modal-confirm(v-if="leaveNoteModalVisible" @modalClose="leaveNoteModalVisible = false;" @modalConfirm="leaveNoteEdit" :message="'Все несохраненные изменения будут утеряны, вы уверены?'" :confirmButtonText="'Выйти'")
</template>
...
export default {
    ...
    data() {
        return {
            ...
            currentFilter: 'activeTasksFilter',
            currentFilterText: 'Активно'
        }
    },
    computed: {
        ...mapState({
        ...
        activeTasksFilter: function() {
            const filtered = this.note.tasks.filter(function(task) {
                return (task.date.getTime() <= new Date().setHours(0, 0, 0, 0)) && !task.completed
            })
            return filtered
        },
    ...
    methods: {
        ...
        deleteSubTask(taskID, subTaskID) {
            const task = this.note.tasks.find(task => task.id === taskID);
